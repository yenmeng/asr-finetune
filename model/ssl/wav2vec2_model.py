...
def compute_entropy(prob, eps=1e-15):
...
def index_put(tensor, indices, value):
    if torch.is_tensor(value):
        tensor[indices] = value.to(tensor.dtype)
    else:
...
class Wav2Vec2Model(nn.Module):
    ...
    def forward(
        ...
        if output_lengths is None:
            output_lengths = self._get_feat_extract_output_lengths(source_lengths)
        ...
        out, layer_results  = self.encoder(
            x,
        ...
        if features_only:
            return {"feature": out, "padding_mask": padding_mask, "feat_lengths": output_lengths}
    ...
    def extract_features(self, source, source_lengths=None, padding_mask=None, mask=False, output_layer=None):
        res = self.forward(
            source, 
            source_lengths,
            padding_mask, 
        ...
        feat_lengths = res["feat_lengths"]
        feat_padding_mask = res["padding_mask"]
        return features, feat_padding_mask, feat_lengths
