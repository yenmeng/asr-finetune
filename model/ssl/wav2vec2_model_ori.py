...
class Wav2Vec2Model(nn.Module):
    ...
    def forward(
        ...
        if features_only:
            return {"feature": out, "padding_mask": padding_mask, "input_feat": ret_feat}
        
        # out, hiddens = self.encoder(x, padding_mask=padding_mask, layer=layer)
    ...
    def extract_features(self, source, source_lengths=None, padding_mask=None, mask=False, output_layer=None, t=None, w=None):
        res = self.forward(
            source, 
            source_lengths
            padding_mask, 
        ...
        input_features = res["input_feat"]
        feat_padding_mask = res["padding_mask"]
        return features, feat_padding_mask, input_features
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, padding_mask=None, layer=None, t=None, w=None):
        x, layer_results, ret_feat = self.extract_features(x, padding_mask, layer, t=t, w=w)

        if self.layer_norm_first and layer is None:
            x = self.layer_norm(x)

        return x, layer_results, ret_feat
    ...
    def extract_features(
        ...
        if padding_mask is not None:
            input_feat = index_put(input_feat, padding_mask, 0)
         
        if self.pos_conv is not None:
            x_conv = self.pos_conv(input_feat.transpose(1, 2))
            x_conv = x_conv.transpose(1, 2)
            input_feat = input_feat + x_conv

        if not self.layer_norm_first:
            input_feat = self.layer_norm(input_feat)

        # pad to the sequence length dimension
        if input_feat.size(0) == 1:
            pad_length = 0
        else:
            input_feat, pad_length = pad_to_multiple(
                input_feat, self.required_seq_len_multiple, dim=-2, value=0
            )
            if pad_length > 0 and padding_mask is None:
                padding_mask = input_feat.new_zeros((input_feat.size(0), input_feat.size(1)), dtype=torch.bool)
                padding_mask[:, -pad_length:] = True
            else:
                padding_mask, _ = pad_to_multiple(
                    padding_mask, self.required_seq_len_multiple, dim=-1, value=True
                )
        input_feat = F.dropout(input_feat, p=self.dropout, training=self.training)

        if t is not None:
            assert w is not None
            assert w >= 0
            f_len = input_feat.size(1)
            input_feat = input_feat[:, max(0, t-w):min(t+w, f_len), :]
            if padding_mask is not None:
                padding_mask = padding_mask[:, max(0, t-w):min(t+w, f_len)]
        input_feat.requires_grad = True
        ...
        return x, layer_results, input_feat
